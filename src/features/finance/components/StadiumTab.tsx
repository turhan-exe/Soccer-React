      ...
      <Card className="border-white/5 bg-slate-900/60 backdrop-blur-sm shadow-xl p-1">
        <div className="relative h-48 w-full overflow-hidden rounded-t-xl bg-slate-950">
          ...
          <div className="absolute bottom-4 left-4 z-20">
            <span className="bg-emerald-500 text-slate-950 text-[10px] font-bold px-2 py-0.5 rounded shadow-lg uppercase tracking-wider mb-1 inline-block">
              Seviye {level}
            </span>
        ...
        <CardContent className="pt-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <StadiumStat
              icon={Users}
              label="Kapasite"
              value={config.capacity.toLocaleString('tr-TR')}
              subValue={`${occupiedSeats.toLocaleString('tr-TR')} aktif seyirci`}
              tone="blue"
            />
            <StadiumStat
              icon={DollarSign}
              ...
              subValue={`Doluluk %${Math.round(attendanceRate * 100)}`}
              tone="emerald"
            />
          </div>
          ...
          {!permissionPending && !permissionGranted && (
            <p className="text-xs text-center text-rose-400 bg-rose-500/10 p-2 rounded">
              Bu islem icin takim sahibi yetkisi gerekir.
            </p>
          )}
        </CardContent>
      </Card>
