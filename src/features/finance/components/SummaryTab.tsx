        ...
        <div className="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4 scrollbar-hide snap-x">
          ...
          <div className="snap-center shrink-0 w-[200px] h-[120px] rounded-[24px] bg-[#1a1b2e] border border-white/5 relative overflow-hidden">
            ...
            <div className="p-5 flex flex-col h-full justify-center">
              ...
              <p className="text-slate-400 text-xs font-medium mb-1">Son 30 Gun Net</p>
              <p
      ...
      <Card className="bg-[#1a1b2e] border-white/5 rounded-[24px] overflow-hidden shadow-xl">
        <CardContent className="p-0">
          <div className="divide-y divide-white/5">
            ...
            />
            <ListItem label="Mac Basi Tahmini Gelir" value={formatCurrency(averageMatchIncome)} valueColor="text-emerald-300" />
            <ListItem label="Aylik Tahmini Gelir" value={formatCurrency(expectedRevenue.monthly)} valueColor="text-emerald-400" />
            <ListItem label="Günlük Tahmini Gelir" value={formatCurrency(dailyIncomeEstimate)} valueColor="text-emerald-300" />
            <ListItem label="Sponsor Geliri (Aylik)" value={formatCurrency(expectedRevenue.sponsorEstimate)} />
            <ListItem label="Takim Gucu" value={expectedRevenue.teamStrength.toString()} />
            <ListItem
...
function ListItem({
  ...
  return (
    <div className="flex items-center justify-between gap-4 p-5 hover:bg-white/5 transition-colors group">
      ...
      <div
        className={`shrink-0 font-mono font-bold tracking-tight bg-black/20 px-3 py-1 rounded-lg border border-white/5 ${valueColor}`}
      >
        {value}
      </div>
    </div>
