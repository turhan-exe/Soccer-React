...
function normalizeKickoffHour(value) {
 ...
 */
export async function assignTeam(teamId, teamName, ownerUid) {
    ...
    let finalizedToScheduleOuter = null;
    const leagueRef = await db.runTransaction(async (tx) => {
        ...
        const existing = await tx.get(db.collectionGroup('teams').where('teamId', '==', teamId).limit(1));
        if (!existing.empty) {
        ...
        const prepareNewFormingLeague = async () => {
            ...
            const last = await tx.get(db.collection('leagues').orderBy('season', 'desc').limit(1));
            const nextSeason = last.empty
                ? 1
                : (last.docs[0].data().season || 0) + 1;
            const newLeagueData = {
        ...
        let chosenLeagueRef = null;
        let chosenLeagueData = null;
...
export const assignTeamToLeagueHttp = functions.region('europe-west1').https.onRequest(async (req, res) => {
    ...
    try {
        ...
        functions.logger.error('[HTTP] assignTeamToLeagueHttp hata', e);
        res.status(500).json({ error: 'internal' });
    }
});
async function generateFixturesForLeague(leagueId, startDate) {
...
export const generateRoundRobinFixturesFn = functions.region('europe-west1').https.onCall(async (request) => {
    requireAppCheck(request);
    requireAuth(request);
    const leagueId = request.data?.leagueId;
    ...
    const startDateSafe = force
        ...
        : (league.startDate?.toDate?.() || nextDayKickoffForLeague(leagueId, league));
