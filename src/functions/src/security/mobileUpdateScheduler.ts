...
const readPendingActivation = (value: unknown) => {
  ...
  const latestVersionName = asString(source.latestVersionName) || String(effectiveLatestVersionCode);
...
export const applyPendingMobileUpdatePolicies = functions
  ...
  .onRun(async () => {
    ...
    functions.logger.info('[mobileUpdatePolicy] applied pending activation', {
